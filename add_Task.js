...
async function updateExistingContacts(isEditMode) {
  const classPrefix = isEditMode ? "edit" : "add";
  let checkboxes = document.querySelectorAll(`.${classPrefix}_task_contact_checkbox`);

  // Leeren Sie die existingContacts Liste
  existingContacts = [];

  // Fügen Sie alle ausgewählten Kontakte zur existingContacts Liste hinzu
  for (let i = 0; i < checkboxes.length; i++) {
    if (checkboxes[i].checked) {
      existingContacts.push(checkboxes[i].value);
    }
  }
}
